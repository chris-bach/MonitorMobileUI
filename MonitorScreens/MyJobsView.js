    ...
    useEffect(() => {
        ...
        jobList.forEach((job, key) => {
            let jobInfo = {
                ...
                jobName:job.jobName,
                address: job.address,
                start: JSON.stringify(job.start[0]).concat("/", JSON.stringify(job.start[1])).concat("/", JSON.stringify(job.start[2])),
                end: JSON.stringify(job.end[0]).concat("/", JSON.stringify(job.end[1])).concat("/", JSON.stringify(job.end[2])),
